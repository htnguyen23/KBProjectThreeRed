...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      // Extend edge distance to existing path
      this.distance = copyPath.distance + extendBy.weight;
      this.dataSequence = new LinkedList<>();
      
      // Make a deep copy of the copyPath dataSequence 
      for(T v: copyPath.dataSequence) {
        dataSequence.add(v);
      }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    Vertex stopNode = vertices.get(end);
    
    // Throw exception when vertex containing start or end cannot be found
    if (source == null || stopNode == null)
      throw new NoSuchElementException(
          String.format("Graph does not contain vertex %s.", source == null? start.toString(): end.toString()));
    
    Path shortestPath = new Path(source);
    pq.add(shortestPath);

    // Analyze all unvisited paths and measure their distances by adding
    // the current distance of the current node to the weight of the edge
    while(!pq.isEmpty()) {
      // Pop shortest discovered path off the pq
      shortestPath = pq.remove();
      // STOP if the destination node has been marked visited
      if(shortestPath.end.equals(stopNode)) return shortestPath;

      // Consider all unvisited neighbors of the current node 
      for(Edge edge: shortestPath.end.edgesLeaving) {
        Vertex neighbor = edge.target;        
        if (visited.contains(neighbor)) continue; 
        // relax edge e and update pq if changed
        if(!visited.contains(neighbor)) {  
          pq.add(new Path(shortestPath, edge));
